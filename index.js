...
app.get('/analytics/passengers', authenticateToken, async (req, res) => {
  try {
    // Only allow admin or users to access analytics
    if (!['admin', 'user'].includes(req.user.role)) {
      return res.status(403).json({ error: 'Unauthorized' });
    }

    const pipeline = [
      // 1. Filter only passenger users
      { $match: { role: 'user' } },

      // 2. Join with rides collection (convert user._id to string for matching)
      {
        $lookup: {
          from: 'rides',
      ...
      // 3. Only include users with at least 1 ride that has a fare
      { $match: { 'userRides.fare': { $exists: true } } },

      // 4. Calculate total rides and fares
      {
        $addFields: {
          totalRides: { $size: '$userRides' },
    ...
    console.error('Error fetching passenger analytics:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
